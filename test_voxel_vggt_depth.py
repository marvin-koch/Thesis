...
for i, images in enumerate(sub_dirs):
    ...
    if i < 1:
        ...
        init_extr = predictions["extrinsic"]
        init_intr = predictions["intrinsic"]
        
        # in iter=0, after predictions = run_model(...)
        depth0_all = predictions["depth"]
        GA_CACHE["depth0_all"] = depth0_all


    else:
        start = time.time()

        changed_idx = changed_images(image_tensors, keyframes, thresh=0.97)
        ...
        for group in groups:
            ...
            new_extr.append(init_extr[group])
            new_intr.append(init_intr[group])
        ...
        decision = {"use_groups": True} #decide_grouping_vs_all(grown_groups, grown_changed_idx, rho=0.9)

        print(decision)
        
        new_vggt_input = new_vggt_input if decision["use_groups"] else [image_tensors[grown_changed_idx]]
        new_extr = new_extr if decision["use_groups"] else [init_extr[grown_changed_idx]]
        new_intr = new_intr if decision["use_groups"] else [init_intr[grown_changed_idx]]
        ...
        def camera_centers_from_Twc(Twc_S_4x4):
            # Cam center in world = translation part for Twc (cam->world)
            return Twc_S_4x4[:, :3, 3]  # (S,3)

        def umeyama_sim3(X, Y, with_scale=True):
            """
            X, Y: (N,3), find s,R,t s.t. Y ≈ s R X + t  (map X->Y)
            Returns s, R, t
            """
            ...
            if with_scale:
                ...
                s = np.trace(np.diag(np.linalg.svd(S, compute_uv=False))) / varX
            else:
                s = 1.0
            t = muY - s * (R @ muX)
            return float(s), R.astype(np.float32), t.astype(np.float32)

        def apply_sim3_to_points(P_hw3, s, R, t):
            return (s * (P_hw3 @ R.T)) + t  # if P are row-vectors

        def apply_sim3_to_Twc(Twc, s, R, t):
            """
            Apply Sim(3) (s,R,t) to Twc (camera->world).
            Twc can be (4,4) or (3,4). Returns same shape as input.
            """
            Twc = np.asarray(Twc, dtype=np.float32)
            squeeze = False

            if Twc.shape[-2:] == (3,4):   # (3,4)
                # pad to homogeneous
                bottom = np.array([[0,0,0,1]], dtype=np.float32)
                Twc = np.vstack([Twc, bottom])
                squeeze = True
            ...
            if squeeze:
                Twc_new = Twc_new[:3,:4]  # return to (3,4)

            return Twc_new.astype(np.float32)


        def camera_centers_from_Twc(Twc_S_4x4):        # (S,4,4) Twc
            return Twc_S_4x4[:, :3, 3].astype(np.float32)


        anchor_local = 0   # by construction you always added [0] first
        anchor_gid = 0  # or whichever gid you chose in iter=0

        stacked_predictions = []
        for gi, input_frames in enumerate(new_vggt_input):
            print(input_frames.size())
            pred = run_model(model, input_frames)
            Ks = (new_intr[gi])    # (S,3,3) for this group
            Ts = (new_extr[gi])    # (S,3,4) for this group
            world_points = unproject_depth_map_to_point_map(pred["depth"], Ts, Ks)
            
            Twc_vggt = pred["extrinsic"]

            if Twc_vggt.shape[-2:] == (3,4):
                S_ = Twc_vggt.shape[0]
                pad = np.tile(np.array([[0,0,0,1]], np.float32), (S_,1,1))
                Twc_vggt = np.concatenate([Twc_vggt.astype(np.float32), pad], axis=1)  # (S,4,4)

            if Ts.shape[-2:] == (3,4):
                S_ = Ts.shape[0]
                pad = np.tile(np.array([[0,0,0,1]], np.float32), (S_,1,1))
                Ts_new = np.concatenate([Ts.astype(np.float32), pad], axis=1)  # (S,4,4)

            # --- Align to your global world with Sim(3) ---
            # camera centers in both coord frames
            C_vggt   = camera_centers_from_Twc(pred["extrinsic"])          # (S,3)
            C_global = camera_centers_from_Twc(Ts_new)  # (S,3) from iter 0 cache

            s,R,t = umeyama_sim3(C_vggt, C_global, with_scale=True)

            # apply to points
            world_points_global = apply_sim3_to_points(
                world_points.reshape(-1,3), s,R,t
            ).reshape(world_points.shape)

           
            # update prediction dict
            pred["world_points_from_depth"] = world_points_global
            Twc_aligned  = np.stack([apply_sim3_to_Twc(pred["extrinsic"][i], s, R, t)    # optional
                                    for i in range(pred["extrinsic"].shape[0])], axis=0)

            # pred["world_points_from_depth"] = world_points

            pred["extrinsic"] = Twc_aligned
            # pred["intrinsic"] = Ks
