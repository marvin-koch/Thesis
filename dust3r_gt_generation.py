...
def get_reconstructed_scene(
    ...
    if itr == 0:
        ...
        output = inference(pairs, model, device, batch_size=2, verbose=not silent)
    ...
    if len(pairs_to_run):
        out_delta = inference(pairs_to_run, model, device, batch_size=2, verbose=not silent)
...
save_root = "bedroom_habitat_dust3r_gt_new/"
...
for i, images in enumerate(sub_dirs):
    ...
    if i < 1:
        ...
        predictions = get_reconstructed_scene(0, ".", imgs, model, device, False, 512, target_dir + images, "linear", 100, 1, True, False, True, False, 0.05, "oneref", 1, 0)
        ...
        for input_frames in [imgs]:
            stacked_predictions.append(get_reconstructed_scene(i, ".", input_frames, model, device, False, 512, target_dir + images, "linear", 100, 1, True, False, True, False, 0.05, "oneref", 1, 0, changed_gids=[i for i in range(10)]))
