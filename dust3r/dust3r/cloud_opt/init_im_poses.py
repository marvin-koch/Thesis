...
def init_from_known(self):
    ...
    print("Running assert took", length, "seconds!")
    start = time.time()
    ...
    for e, (i, j) in enumerate(tqdm(self.edges, disable=not self.verbose)):
        ...
        # remember if this is a good depthmap
        score = float(self.conf_i[i_j].mean())
        s = self.get_pw_scale()[e]
        if score > best_depthmaps.get(i, (0,))[0]:
            best_depthmaps[i] = score, i_j, s
    end = time.time()
    length = end - start

    print("Running for took", length, "seconds!")
    ...
    print("Running setting took", length, "seconds!")
