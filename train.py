...
class VoxelUpdaterSystem(pl.LightningModule):
    ...
    def compute_tfs(prev_keys, prev_binary, curr_keys, curr_binary):
        ...
        Keys may be 1D (pre-hashed) or 2D (N,3) integer coords.
        """
        if prev_keys is None or curr_keys is None:
            return None

        # Support both 1D hashed keys and 2D (N,3) coordinate keys
        if prev_keys.ndim == 2:
            M = 1000003
            ...
            prev_hash = prev_keys.long()
            curr_hash = curr_keys.long()
